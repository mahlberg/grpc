...
class Stream : public RefCounted<Stream> {
 public:
  ...
  void CancelCall(absl::Status&& status) {
    ...
    // Avoid store/exchange invalidation if already cancelled.
    if (did_cancel_.load(std::memory_order_acquire)) return;
    if (did_cancel_.exchange(true, std::memory_order_acq_rel)) return;
    if (is_client()) {
